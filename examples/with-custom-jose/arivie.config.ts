...
export async function getArivieRuntimeForOwner(ownerId: string): Promise<{ arivie: ArivieApp }> {
  ...
  if (cached == null) {
    ...
    const config: ArivieAppConfig = {
      app: { id: ownerId, name: "With Custom Jose Example" },
      model: resolveModel(),
      ...
      agents: {
        analyst: defineAgent({
          instructions: "Answer analytics questions for the custom JOSE example.",
...
export async function getArivieRuntime(): Promise<{ arivie: ArivieApp }> {
  return getArivieRuntimeForOwner(process.env.ARIVIE_OWNER_ID ?? "with-custom-jose-owner");
}
