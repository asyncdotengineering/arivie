...
const PORT = Number(process.env.ARIVIE_SMOKE_PORT ?? 3110);
...
async function assertMockAnswer(headers: Record<string, string>): Promise<void> {
  ...
  if (!text.includes(MOCK_SUBSTRING)) {
    throw new Error(`Missing mock substring: ${text.slice(0, 800)}`);
  }
  console.log("boot:smoke OK — mock response verified");
}

function startNext(ownerId: string): ChildProcess {
  const root = join(dirname(fileURLToPath(import.meta.url)), "..");
