...
function resolveModel(): LanguageModel {
  ...
  console.warn(
    "[with-clerk] No model key set (GOOGLE_GENERATIVE_AI_API_KEY|ANTHROPIC_API_KEY|OPENAI_API_KEY) — using deterministic mock model.",
  );
...
export async function getArivieRuntime(): Promise<{
  ...
  return getArivieRuntimeForOwner(
    process.env.ARIVIE_OWNER_ID ?? "with-clerk-owner",
  );
}
