...
export async function getArivieRuntime(): Promise<{ arivie: ArivieApp }> {
  if (cached == null) {
    ...
    const ownerId = process.env.ARIVIE_OWNER_ID ?? "with-hono-owner";
    const config: ArivieAppConfig = {
      app: { id: ownerId, name: "With Hono Example" },
      model: resolveModel(),
      ...
      agents: {
        analyst: defineAgent({
          instructions: "Answer analytics questions for the Hono example.",
