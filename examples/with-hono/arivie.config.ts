...
function resolveModel(): LanguageModel {
  ...
  console.warn(
    "[with-hono] No model key set (GOOGLE_GENERATIVE_AI_API_KEY|ANTHROPIC_API_KEY|OPENAI_API_KEY) — using deterministic mock model.",
  );
...
export async function getArivieRuntime(): Promise<{
  ...
  if (cached == null) {
    ...
    const arivie = await defineArivie({
      owner: {
        id: process.env.ARIVIE_OWNER_ID ?? "with-hono-owner",
        name: "With Hono Example",
      },
      model: resolveModel(),
    ...
    const mcp = makeMcpServer({
      ...
      semantic,
      db: postgres,
      ownerId: process.env.ARIVIE_OWNER_ID ?? "with-hono-owner",
      ownerName: "With Hono Example",
    });
