...
function resolveModel(): LanguageModel {
  ...
  console.warn(
    "[with-better-auth] No model key set (GOOGLE_GENERATIVE_AI_API_KEY|ANTHROPIC_API_KEY|OPENAI_API_KEY) — using deterministic mock model.",
  );
...
export async function getArivieRuntimeForOwner(ownerId: string): Promise<{
  ...
  if (cached == null) {
    ...
    const mcp = makeMcpServer({
      ...
      db: postgres,
      ownerId,
      ownerName: "With Better Auth Example",
    });
    cached = { arivie, mcp };
...
export async function getArivieRuntime(): Promise<{
  ...
  return getArivieRuntimeForOwner(
    process.env.ARIVIE_OWNER_ID ?? "with-better-auth-owner",
  );
}
