...
function resolveModel(): LanguageModel {
  ...
  if (googleKey != null && googleKey.length > 0) {
    return createGoogleGenerativeAI({ apiKey: googleKey })(process.env.GOOGLE_MODEL ?? "gemini-2.5-flash") as LanguageModel;
  }
...
export async function getArivieRuntime(): Promise<{ arivie: ArivieApp }> {
  if (cached == null) {
    ...
    const ownerId = process.env.ARIVIE_OWNER_ID ?? "with-bun-owner";
    const config: ArivieAppConfig = {
      app: { id: ownerId, name: "With Bun Example" },
      model: resolveModel(),
      ...
      agents: {
        analyst: defineAgent({
          instructions: "Answer analytics questions for the Bun example.",
