...
describe("wrapInstructionsForCache", () => {
  ...
  it("promotes experimental_providerMetadata to providerOptions on the system model message via Mastra's pipeline", () => {
    const model = new MockLanguageModelV3({
      provider: "anthropic",
      modelId: "claude-sonnet-4-20250514",
    });
    const wrapped = wrapInstructionsForCache("You are an analyst.", model);
    const list = new MessageList();
    list.addSystem([wrapped]);
    const prompt = list.all.aiV5.prompt();
