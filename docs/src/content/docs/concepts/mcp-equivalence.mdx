...
`@arivie/mcp` wraps `makeMcpServer` from Mastra's MCP primitive (`@mastra/mcp`). The Next.js example mounts HTTP at `app/api/chat/route.ts` (built-in `POST /api/chat`) and MCP at `app/api/arivie/mcp/route.ts`, both from `getArivieRuntime()` in `arivie.config.ts`.
...
Use **HTTP** when your UI uses `ArivieChat` from `@arivie/react/chat` or posts to the built-in `POST /api/chat` via `arivie.handler`. Use **MCP** when Claude Desktop, Cursor, or another MCP host should discover analytics tools. Configuration differs only at the transport edge; `app.id`, `semanticPath`, and model resolver stay in `arivie.config.ts`.
