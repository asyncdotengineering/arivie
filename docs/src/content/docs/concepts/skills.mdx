...
Skills load from `skills/*/SKILL.md` on disk. Add them via CLI:

```bash
pnpm dlx arivie add skill cohort-analysis
```

Reference a skill by name in prompts or schedule config — the agent reads matching `SKILL.md` playbooks from the skills directory. See `examples/with-pos-fnb/skills/` for ten production F&B SOP skills.
