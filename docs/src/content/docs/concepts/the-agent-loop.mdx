...
| `mastra_workspace_*` | When workspace is wired in deployment | Read/write/grep/edit/list/mkdir/delete/stat files in the sandbox. |
| `workspace_bash` | When bash is opted in | Shell access for `jq`, `awk`, `python3`, etc. inside the sandbox. |
| Skill tools | When skills directory is present | Load SOP playbooks from `skills/*/SKILL.md`. |
